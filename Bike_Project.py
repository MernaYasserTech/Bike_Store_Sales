...
page = st.sidebar.selectbox("Choose a page", ["Home", "Data Overview","Customer Age Analysis", "Order Quantity Analysis", "Sales Analysis","Geographical Analysis"])
 #Page 1
if page == "Home":
    st.title("Introduction to Global Bike Store Sales Dataset")
    st.snow()    
...
###Page 3
if page == "Order Quantity Analysis":
...
# Histogram
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Order Quantity Histogram", "Box Plot for Order Quantity"))

# Histogram
    histogram_trace = go.Histogram(x=df['Order_Quantity'], nbinsx=32, marker=dict(color='lightblue', line=dict(color='black', width=1)))
    fig.add_trace(histogram_trace, row=1, col=1)
    fig.update_xaxes(title_text='Order Quantity', row=1, col=1)
    fig.update_yaxes(title_text='Order Frequency', row=1, col=1)

# Box Plot
    box_trace = go.Box(y=df['Order_Quantity'], boxpoints='all', jitter=0.3, pointpos=-1.8, marker=dict(color='lightgreen'), line=dict(color='darkgreen'))
    fig.add_trace(box_trace, row=2, col=1)
    fig.update_xaxes(title_text='Order Quantity', row=2, col=1)
...
# PAge 4
    ...
    st.multiselect("select",['Date','Customer_Gender','Country'])  
